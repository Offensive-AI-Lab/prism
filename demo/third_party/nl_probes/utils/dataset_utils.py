...
def create_training_datapoint(
    ...
    # transformers 5.x returns BatchEncoding (UserDict, not a dict subclass) for VLM tokenizers
    if not isinstance(input_prompt_ids, list):
        input_prompt_ids = input_prompt_ids["input_ids"]
    if not isinstance(input_prompt_ids, list):
        raise TypeError(f"Expected list of token ids from tokenizer, got {type(input_prompt_ids)}")
    ...
    if not isinstance(full_prompt_ids, list):
        raise TypeError(f"Expected list of token ids from tokenizer, got {type(full_prompt_ids)}")
