...
VLM_TEXT_ONLY_LORA_TARGETS = {
    ...
    # Qwen3.5 has a hybrid architecture: 8 full-attention layers (with q/k/v/o_proj)
    # plus 24 linear-attention (gated-delta-net) layers whose projections are named
    # linear_attn.{in_proj_qkv,in_proj_a,in_proj_b,in_proj_z,out_proj}. Including
    # them brings adapted-module coverage from ~50% to ~98% of the language tower,
    # matching the published Qwen3-8B oracle recipe in spirit.
    "qwen3.5": (
    ...
    # Ministral-3 (Mistral3ForConditionalGeneration) is a VLM with a vision tower.
    # Decoder layers live under model.language_model.* with standard attention/MLP
    # projection names, so target only the language tower.
    "ministral": r"model\.language_model\..*\.(q_proj|k_proj|v_proj|o_proj|gate_proj|up_proj|down_proj)",
}
...
def get_hf_submodule(model: AutoModelForCausalLM, layer: int, use_lora: bool = False):
    ...
    if use_lora:
        if "pythia" in model_name:
            ...
            # Ministral-3 VLM: decoder layers under model.model.language_model
            return model.base_model.model.model.language_model.layers[layer]
